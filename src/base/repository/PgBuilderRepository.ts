...
class PgBuilderRepository {
        ...
        switch (orderArgsType) {
            ...
            case 'object':
                if (isSortArray && isArgsArray && orderArgs.length === sort.length) {
                    this.orderByQuery = `\nORDER BY ${orderArgs
                        .map(() => this.replaceChar)
                        .join(', ')} ${sort.map(() => this.replaceChar).join(', ')}`
    ...
    protected getSQL(): string {
        ...
        let index = 0
        const query = this.questionMarkQuery.replaceAll(
            this.replaceChar,
            () => this.params[index++]
        )
    ...
    private generateAllQueryAndParams(): { query: string; params: any[] } {
        ...
        if (this.queryString || this.queryParams.length) this.params = this.queryParams
            ...
            if (this.safeDeleteParams.length)
                this.params = [...this.params, ...this.safeDeleteParams]
